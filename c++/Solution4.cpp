...
nums1=1239
nums2=123456789
總長=13, 中位數index=half=6, 代表左側需要放6個 -> [ 左邊6個 | 右邊7個 ]
...
nums2=1234  56789
            j
其中nums1[i-1]<nums2[j], 但是nums2[j-1]>nums1[i], j太大了, i太小了
所以將l左移讓i變大, 使l=i+1
l=3, r=4
i=(3+4)/2=3
j=6-3=3
...
nums2=123  456789
           j
此時nums1[i-1]<nums2[j] && nums2[j-1]<nums1[i], 已經抓到前最小的6個
奇數所以答案是右邊(較長邊)的最小值
得result=min(nums1[i], nums2[j])=4

偶數的情況就是左右兩側分別取最大與最小值, 再平均
